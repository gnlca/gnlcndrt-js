...
export async function getStaticProps() {
  ...
  const posts = utils.formatPostsToIds(
    await notion.getPosts(notion.databaseId)
  );
...
export default function Blog({ tags, posts }: any) {
  ...
  return (
    <div className="Blog fullHeight maxWidth42 mxAuto">
      <Head>
      ...
      <div className="content">
        ...
        {postsTags ? (
          <div className="tags">
            <button
              className={activeTags.length == 0 ? "tag hidden" : "tag"}
              onClick={() => {
                setActiveTags([]);
                setActivePosts(posts);
              }}
            ...
            {postsTags.map((tag: any) => (
              <button
                key={tag.id}
                className={
                  activeTags.indexOf(tag.name) == -1 ? "tag" : "tag tagActive"
                }
                type="button"
        ...
        {postsTitles ? (
          <ul>
            {postsTitles.map((titolo: any, index: number) => (
              <li key={index}>
                <Link
                  key={titolo.id}
                  href={`/blog/${titolo.name.replace(/\s+/g, "-")}_${
                    titolo.id
                  }`}
                  passHref
                >
                  <a
                    onClick={() =>
                      console.log(
                        `apro la pagina ${titolo.name} con id ${titolo.id}`
                      )
                    }
                  >
                    {titolo.name}
